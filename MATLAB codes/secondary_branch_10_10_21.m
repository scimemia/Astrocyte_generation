function secondary_branch_10_10_21(u,v,level)
global W r0 R c0 d N Csholl Nsholl step h0 h1 Hcount G_sec L_sec tipdiam
global NT SA ST VOL LGTH brch brch_th Ntip Stip SAtip VOLtip;
global D d diam col ct td thold klength;
...
%this segment is level+1 away from the soma;
        ...
        %add the segment to the sholl profile;
          ...
          VOL = VOL - pi*hbp*diam(1)^2/4 + 1/3*pi*hbp*(D(level)^2+Delta^2+D(level)*Delta);
...
f = 1+0.5*rand;
        ...
        brch = brch + 1;
        if (diam(i)/((1+f^(3/2))^(2/3))> thold) && (csi>thold)
           brch_th = brch_th + 1;
        end
        ...
        %in a random direction, angle theta with the mother branch direction, and length csi;
            ...
            gen(2*i+1) = gen(i)+1;
            if (diam(2*i+1)>thold) && (csi>thold)
                gen_th(2*i+1) = gen_th(i)+1;
            end
            ...
            for sect = 1:1:Nsholl-1
                Rsholl = r0 + sect*step;
                if (v(1)^2+v(2)^2+v(3)^2 < Rsholl^2) && (xa(1)^2+xa(2)^2+xa(3)^2 > Rsholl^2) && (diam(2*i+1)>thold) && (csi>thold)
                     Csholl(sect) = Csholl(sect)+1;
                end
            end
            ...
            if (diam(2*i+1)/((1+f^(3/2))^(2/3))>td)
                ...
                VOL = VOL - pi*hbp*diam(2*i+1)^2/4 + 1/3*pi*hbp*(diam(i)^2+Delta^2+diam(i)*Delta);
            ...
            %add the segment measures to the total counts;
                ...
                VOL = VOL + pi*(csi-diam(2*i+1)/2)*diam(2*i+1)^2/4;
        ...
        %in the same direction as the mother branch;     
            ...
            for sect = 1:1:Nsholl-1
                Rsholl = r0 + sect*step;
                   if (v(1)^2+v(2)^2+v(3)^2 < Rsholl^2) && (xb(1)^2+xb(2)^2+xb(3)^2 > Rsholl^2)&& (diam(2*i)>thold) && (csi>thold)
                      Csholl(sect) = Csholl(sect)+1;
                   end
            end
            ...
            tipdiam(ct) = diam(2*i);
                        ...
                        VOL = VOL + pi*(csi-diam(2*i)/2)*diam(2*i)^2/4;
